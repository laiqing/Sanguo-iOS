...
@implementation WizardInfoLayer

//int const MAXLEVEL = 5;
int const goldcost8[] = {0,4000};
int const woodcost8[] = {0,10};
int const ironcost8[] = {0,20};
//int const goldprovide8[] = {0,0};
//int const woodprovide8[] = {0,0};
//int const ironprovide8[] = {0,0};
...
-(id) init
{
    if ((self=[super init])) {
        ...
        building = [CCSprite spriteWithSpriteFrameName:@"wizard.png"];
        building.scale = 2.0f;
        ...
        //add text desc
        desc = [CCSprite spriteWithSpriteFrameName:@"wizarddesc.png"];
        ...
        CCSprite* sp = [CCSprite spriteWithSpriteFrameName:@"troop7_right_01.png"];
...
-(void) setCityID:(int)cid
{
    ...
    _currentLevel = cio.wizard;
    _cityLevel = cio.level;
    
    //add title "city hall level"
    if (_currentLevel > 0) {
        NSString* tstr = [NSString stringWithFormat:@"学院"];
        ...
        // == 0
        title = [CCLabelTTF labelWithString:@"学院  未建造" fontName:@"Verdana" fontSize:14];
        title.color = ccc3(215, 255, 0);
        title.position = ccp(wsize.width*0.5, bg.position.y+bg.boundingBox.size.height*0.5-12);
        [self addChild:title z:1];
    }
    ...
    if (_currentLevel==0) {
        ...
        NSString* ngstr = [NSString stringWithFormat:@"%d",goldcost8[1]];
        ...
        NSString* nwstr = [NSString stringWithFormat:@"%d",woodcost8[1]];
        ...
        NSString* nistr = [NSString stringWithFormat:@"%d",ironcost8[1]];
...
-(void) touchUpgrade
{
    if (_currentLevel<1) {
        //if money enough
        int needgold = goldcost8[1];
        int needwood = woodcost8[1];
        int neediron = ironcost8[1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:5 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
