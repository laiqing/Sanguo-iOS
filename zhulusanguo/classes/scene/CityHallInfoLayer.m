...
-(void) touchUpgrade
{
    if (_currentLevel<5) {
        //if money enough
        int needgold = goldcost[_currentLevel+1];
        int needwood = woodcost[_currentLevel+1];
        int neediron = ironcost[_currentLevel+1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:1 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
