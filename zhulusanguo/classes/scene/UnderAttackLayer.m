...
#import "UnderAttackLayer.h"


@implementation UnderAttackLayer
...
- (id) initSlidingLayer:(SlideDirection) slideDirection contentRect:(CGRect)contentRect withTargetCityID:(int)tcid{
    if ((self = [super init])) {
        ...
        //only show the heroes in this city
        int kid = [ShareGameManager shareGameManager].kingID;
        NSArray* heroes = [[ShareGameManager shareGameManager] getHeroListFromCity:tcid kingID:kid];
        ...
        if (len>0) {
            ...
            //gold text
            CCLabelTTF* ggtext = [CCLabelTTF labelWithString:@"可参加战斗的武将" fontName:@"Arial" fontSize:12];
            ggtext.anchorPoint = ccp(0, 0.5);
            ggtext.position = ccp(wsize.width*0.5, bottom.position.y);
...
- (void) ccTouchEnded:(UITouch*)touch withEvent:(UIEvent *)event {
    ...
    if (bounceDistance != 0) {
        ...
        __block UnderAttackLayer* fl = self;
