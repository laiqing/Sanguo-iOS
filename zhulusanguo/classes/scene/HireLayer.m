...
- (id) initcontentRect1:(CGRect)left contentRect2:(CGRect)right withCityID:(int)tcid
{
    if ((self=[super init])) {
        ...
        itembg = [CCSprite spriteWithSpriteFrameName:@"itemlist.png"];
        ...
        CCLabelTTF* itemtitle = [CCLabelTTF labelWithString:@"本城流浪武将" fontName:@"Arial" fontSize:12];
        ...
        NSArray* heroes = [[ShareGameManager shareGameManager] getUnemploymentHeroListFromCity:_cityID];
...
-(void) showHeroDetail:(NSNumber *)hid
{
    ...
    //add a hire button , and cost money sprite
    int cost = 2500 + (ho.level-1)*500;
    hirecost = cost;
    CCSprite* gg = [CCSprite spriteWithSpriteFrameName:@"gold.png"];
    gg.scale = 0.4;
    gg.position = ccp(rsp4.position.x+10, rsp4.position.y - 45);
    [rightLayer addChild:gg z:1];
    
    CCLabelTTF* coststr = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%d",cost] fontName:@"Arial" fontSize:10];
    coststr.anchorPoint = ccp(0, 0.5);
    coststr.position = ccp(gg.position.x + 12, gg.position.y);
    [rightLayer addChild:coststr z:1];
    
    TouchableSprite* hirebtn = [TouchableSprite spriteWithSpriteFrameName:@"buildhirebtn.png"];
    hirebtn.position = ccp(gg.position.x + 75 , gg.position.y);
    [rightLayer addChild:hirebtn z:1];
    [hirebtn initTheCallbackFunc:@selector(hireHeroForID:) withCaller:self withTouchID:ho.heroID];
    effpos = hirebtn.position;
    
}

-(void) playHireEffect
{
    CGPoint startP = effpos;
    ...
    CCLabelTTF* goldmvtxt = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%d",hirecost] fontName:@"Arial" fontSize:10];
...
{
    //for touchablebutton callback
    int hidval = (int)[hid integerValue];
    //if money enough
    HeroObject* ho = [[ShareGameManager shareGameManager] getHeroInfoFromID:hidval];
    int cost = 2500 + (ho.level-1)*500;
    int total = [ShareGameManager shareGameManager].gold;
    if (total >= cost) {
        [[SimpleAudioEngine sharedEngine] playEffect:@"sthget.caf"];
        // - cost
        [ShareGameManager shareGameManager].gold -= cost;
        
        //update db set hero owner id
        int kid = [ShareGameManager shareGameManager].kingID;
        [[ShareGameManager shareGameManager] hireHeroWithID:hidval forKing:kid];
        
        //update main change resource label
        ...
        //remove this hero from left list
        ...
        CCLabelTTF* warn = [CCLabelTTF labelWithString:@"没有足够的金钱来聘用此武将！" fontName:@"Arial" fontSize:15];
        warn.color = ccYELLOW;
...
{
    [rightLayer removeAllChildrenWithCleanup:YES];
    [leftLayer removeAllChildrenWithCleanup:YES];
    //clear the left list , then reload again
