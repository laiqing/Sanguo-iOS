...
@implementation CavalryInfoLayer


//int const MAXLEVEL = 5;
int const goldcost7[] = {0,5000};
int const woodcost7[] = {0,15};
int const ironcost7[] = {0,15};
//int const goldprovide7[] = {0,0};
//int const woodprovide7[] = {0,0};
//int const ironprovide7[] = {0,0};
...
-(id) init
{
    if ((self=[super init])) {
        ...
        building = [CCSprite spriteWithSpriteFrameName:@"cavalry.png"];
        building.scale = 2.0f;
        ...
        //add text desc
        desc = [CCSprite spriteWithSpriteFrameName:@"cavalrydesc.png"];
        ...
        CCSprite* sp = [CCSprite spriteWithSpriteFrameName:@"troop5_right_01.png"];
...
-(void) onExit
{
    CCLOG(@"call cavalry layer onExit()....");
...
-(void) setCityID:(int)cid
{
    ...
    _currentLevel = cio.cavalry;
    _cityLevel = cio.level;
    
    //add title "city hall level"
    if (_currentLevel > 0) {
        NSString* tstr = [NSString stringWithFormat:@"骑兵营"];
        ...
        // == 0
        title = [CCLabelTTF labelWithString:@"骑兵营  未建造" fontName:@"Verdana" fontSize:14];
        title.color = ccc3(215, 255, 0);
        title.position = ccp(wsize.width*0.5, bg.position.y+bg.boundingBox.size.height*0.5-12);
        [self addChild:title z:1];
    }
    ...
    if (_currentLevel==0) {
        ...
        NSString* ngstr = [NSString stringWithFormat:@"%d",goldcost7[1]];
        ...
        NSString* nwstr = [NSString stringWithFormat:@"%d",woodcost7[1]];
        ...
        NSString* nistr = [NSString stringWithFormat:@"%d",ironcost7[1]];
...
-(void) touchUpgrade
{
    if (_currentLevel<1) {
        //if money enough
        int needgold = goldcost7[1];
        int needwood = woodcost7[1];
        int neediron = ironcost7[1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:4 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
