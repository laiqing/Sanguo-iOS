...
@implementation BlacksmithInfoLayer



//int const MAXLEVEL = 5;
int const goldcost9[] = {0,6000};
int const woodcost9[] = {0,20};
int const ironcost9[] = {0,20};
//int const goldprovide9[] = {0,0};
//int const woodprovide9[] = {0,0};
//int const ironprovide9[] = {0,0};
...
-(id) init
{
    if ((self=[super init])) {
        ...
        building = [CCSprite spriteWithSpriteFrameName:@"blacksmith.png"];
        building.scale = 2.0f;
        ...
        //add text desc
        desc = [CCSprite spriteWithSpriteFrameName:@"blacksmithdesc.png"];
        ...
        CCSprite* sp = [CCSprite spriteWithSpriteFrameName:@"troop9_right_01.png"];
...
-(void) onExit
{
    CCLOG(@"call wizard layer onExit()....");
...
-(void) setCityID:(int)cid
{
    ...
    _currentLevel = cio.blacksmith;
    _cityLevel = cio.level;
    
    
    //add title "city hall level"
    if (_currentLevel > 0) {
        NSString* tstr = [NSString stringWithFormat:@"铁匠铺"];
        ...
        // == 0
        title = [CCLabelTTF labelWithString:@"铁匠铺  未建造" fontName:@"Verdana" fontSize:14];
        title.color = ccc3(215, 255, 0);
        title.position = ccp(wsize.width*0.5, bg.position.y+bg.boundingBox.size.height*0.5-12);
        [self addChild:title z:1];
    }
    ...
    if (_currentLevel==0) {
        ...
        NSString* ngstr = [NSString stringWithFormat:@"%d",goldcost9[1]];
        ...
        NSString* nwstr = [NSString stringWithFormat:@"%d",woodcost9[1]];
        ...
        NSString* nistr = [NSString stringWithFormat:@"%d",ironcost9[1]];
        ...
        //add forge btn
        forgeBtn = [TouchableSprite spriteWithSpriteFrameName:@"buildforgebtn.png"];
...
-(void) touchUpgrade
{
    if (_currentLevel<1) {
        //if money enough
        int needgold = goldcost9[1];
        int needwood = woodcost9[1];
        int neediron = ironcost9[1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:6 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
