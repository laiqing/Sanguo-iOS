...
@implementation TavernInfoLayer


//int const MAXLEVEL = 5;
int const goldcost11[] = {0,1000};
int const woodcost11[] = {0,10};
int const ironcost11[] = {0,0};
...
-(id) init
{
    if ((self=[super init])) {
        ...
        building = [CCSprite spriteWithSpriteFrameName:@"tavern.png"];
        building.scale = 2.0f;
        ...
        //add text desc
        desc = [CCSprite spriteWithSpriteFrameName:@"taverndesc.png"];
...
-(void) onExit
{
    CCLOG(@"call tavern layer onExit()....");
...
-(void) setCityID:(int)cid
{
    ...
    _currentLevel = cio.tavern;
    _cityLevel = cio.level;
    
    //add title "city hall level"
    if (_currentLevel > 0) {
        NSString* tstr = [NSString stringWithFormat:@"招贤馆"];
        ...
        // == 0
        title = [CCLabelTTF labelWithString:@"招贤馆  未建造" fontName:@"Verdana" fontSize:14];
        title.color = ccc3(215, 255, 0);
        title.position = ccp(wsize.width*0.5, bg.position.y+bg.boundingBox.size.height*0.5-12);
        [self addChild:title z:1];
    }
    ...
    if (_currentLevel==0) {
        ...
        NSString* ngstr = [NSString stringWithFormat:@"%d",goldcost11[1]];
        ...
        NSString* nwstr = [NSString stringWithFormat:@"%d",woodcost11[1]];
        ...
        NSString* nistr = [NSString stringWithFormat:@"%d",ironcost11[1]];
        ...
        //add recruit btn
        otherBtn = [TouchableSprite spriteWithSpriteFrameName:@"buildhirebtn.png"];
...
-(void) touchUpgrade
{
    if (_currentLevel<1) {
        //if money enough
        int needgold = goldcost11[1];
        int needwood = woodcost11[1];
        int neediron = ironcost11[1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:11 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
