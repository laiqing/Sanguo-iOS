...
@implementation ArcherInfoLayer


//int const MAXLEVEL = 5;
int const goldcost6[] = {0,2000};
int const woodcost6[] = {0,10};
int const ironcost6[] = {0,5};
//int const goldprovide6[] = {0,0};
//int const woodprovide6[] = {0,0};
//int const ironprovide6[] = {0,0};
...
-(id) init
{
    if ((self=[super init])) {
        ...
        building = [CCSprite spriteWithSpriteFrameName:@"archer.png"];
        building.scale = 2.0f;
        ...
        //add text desc
        desc = [CCSprite spriteWithSpriteFrameName:@"archerdesc.png"];
...
-(void) setCityID:(int)cid
{
    ...
    _currentLevel = cio.archer;
    _cityLevel = cio.level;
    
    //add title "city hall level"
    if (_currentLevel > 0) {
        NSString* tstr = [NSString stringWithFormat:@"弓兵营"];
        ...
        // == 0
        title = [CCLabelTTF labelWithString:@"弓兵营  未建造" fontName:@"Verdana" fontSize:14];
        title.color = ccc3(215, 255, 0);
        title.position = ccp(wsize.width*0.5, bg.position.y+bg.boundingBox.size.height*0.5-12);
        [self addChild:title z:1];
    }
    ...
    if (_currentLevel==0) {
        ...
        NSString* ngstr = [NSString stringWithFormat:@"%d",goldcost6[1]];
        ...
        NSString* nwstr = [NSString stringWithFormat:@"%d",woodcost6[1]];
        ...
        NSString* nistr = [NSString stringWithFormat:@"%d",ironcost6[1]];
...
-(void) touchUpgrade
{
    if (_currentLevel<1) {
        //if money enough
        int needgold = goldcost6[1];
        int needwood = woodcost6[1];
        int neediron = ironcost6[1];
        if (([ShareGameManager shareGameManager].gold >= needgold)&&([ShareGameManager shareGameManager].wood>= needwood)&&([ShareGameManager shareGameManager].iron >= neediron)) {
            ...
            [[ShareGameManager shareGameManager] upgradeCityBuilding:3 withNewLevel:_currentLevel+1 withCityID:_cityID withNewCityLevel:_cityLevel+1];
